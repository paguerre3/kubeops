...
- Deployment and StatefulSet are based on the same Container specification and they both can handle Storage/Volumes the same way so, why is StatefulSet needed for? replicating stateFUL applications is more difficult and it has different requirements than stateLESS applications, i.e. because Database Pods can't be created/deletead at "same" time and they can't be "randomly" addressed/identified because replica Pods aren't identical (they have their own Pod Identity). In other words, what StatefulSet provides in difference to Deployment is the fact of giving a Pod its own Identity. 