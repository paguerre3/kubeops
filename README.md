...
- Service=<code>ports</code>:<code><code>port</code> is the Port that the service uses to forward requests among pods and the Service=<code>ports</code>:<code><code>targetPod</code> is actually the Port where the container is running therefore it usually matches with Deployment=<code>spec</code>:<code>template</code>:<code>spec</code>:<code>ports</code>:<code>containerPort</code>, e.g.
